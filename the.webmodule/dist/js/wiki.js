...
qwiki.preprocess = function(processor){
	...
	processor.escapedHtml = {};
	var htmlIdx = 1;
	processor.text = processor.text.replace(/\[\[html\]\]([\s\S]+?)\[\[\/html\]\]/g,function(){
		var html = arguments[1];
		var id = htmlIdx++;
		processor.escapedHtml[id] = html;
		return "HTML_BLOCK_"+id;
	});
...
qwiki.create = function(text, logwriter){
	var result = {
		...
		process : function(){
			...
			this.html = this.html.replace(/HTML_BLOCK_(\d+)/g,function(){
				var id = arguments[1];
				var html = self.escapedHtml[Number(id)];
				return html;
			});
		}
	};
