...
define(["the","chai"],function($the,chai) {
    ...
    describe("the.collections.linq", function () {
        ...
        describe("is LINQ like", function () {
            ...
            it("supports sum([simpleexpr])", function () {
                var i = $(5);
                i.sum().should.equal(10);
                i.where(">1").sum().should.equal(9);
                i.where(">1").sum("*2").should.equal(18);
            });
            it("supports sum([prop])", function () {
                var i = $([{x:1},{x:2},{x:3}]);
                i.sum(".x").should.equal(6);
                i.sum(".x*2").should.equal(12);
            });
