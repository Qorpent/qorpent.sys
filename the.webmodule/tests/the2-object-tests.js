 ...
 */
define(["the2","chai"],function($the,chai){
    var $ = $the.Extender;
    var should= chai.Should();
    describe("the2.object", function () {
        this.timeout(5000);
        ...
        describe("#cast", function () {
            ...
            it("can cast from object with extensions", function () {
                $.cast(t, {A: 1, B: 2, C: 3}, $.CastOptions.Extend).should.instanceOf(t);
                $.cast(t, {A: 1, B: 2, C: 3}, $.CastOptions.Extend).should.eql({a: 1, b: 2, C: 3});
            });
        ...
        it("#isUserDefined", function () {
            var f = function () {
            };
            $.isUserDefined({}).should.equal(true);
            $.isUserDefined({x: 1}).should.equal(true);
            $.isUserDefined(new f()).should.equal(true);
            $.isUserDefined(/x/).should.equal(false);
            $.isUserDefined(new Date()).should.equal(false);
            $.isUserDefined("2").should.equal(false);
            $.isUserDefined(2).should.equal(false);
            $.isUserDefined(null).should.equal(false);
