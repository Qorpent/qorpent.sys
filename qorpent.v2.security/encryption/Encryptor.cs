...
namespace qorpent.v2.security.encryption {
    public class Encryptor : IEncryptProvider {
        ...
        public string Encrypt(byte[] data) {
            var result =
                Uri.EscapeDataString(Convert.ToBase64String(Transform(data, _cryptoService.CreateEncryptor(Key, Vector))));
            return result;
        }

        // vector and key have to match between encryption and decryption
        public string Decrypt(string text) {
            var data = DecryptData(text);
        ...
        public byte[] DecryptData(string text) {
            ...
            var data = Transform(str, _cryptoService.CreateDecryptor(Key, Vector));
            return data;
        }
