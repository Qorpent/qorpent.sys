...
namespace qorpent.v2.security.messaging.queues
{
    ...
    public class ElasticSearchMessageQueue:ServiceBase,IMessageQueue
    {
        ...
        public void InitializeFromXml(XElement x) {
                ...
                foreach (var element in e.Elements("queue")) {
                    if (element.Attr("code") == "elastic") {
                        ...
                        if (!string.IsNullOrWhiteSpace(_ref)) {
                            ...
                            if(null==_e)throw new Exception("invalid ref");
                            _e = _e.Element(_ref);
                            if (null == _e) throw new Exception("invalid ref");
                            ReadSingle(_e);
                        }
        ...
        public PostMessage PushMessage(PostMessage message) {
            ...
            {
                var json = message.stringify()+Guid.NewGuid().ToString();
        ...
        private const string getNotSendMessages = @"{""size"":${size},""query"":{""bool"":{""must"":[{""term"":{""WasSent"":{""value"":""false""}}},{""range"":{""StartTime"":{""lte"":""now""}}}]}}}";
