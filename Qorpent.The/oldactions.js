...
*/
define(['./the-jsonify'], function ($the) {
    return $the(function ($root) {
    qObject = $root;
	var planalize = qObject.jsonify;
	var planalizeOptions = qObject.jsonify.JsonifyOptions;
	var extend = qObject.extend;
	var extensionsRegex = /h[qd]_/;
	var applyHttpExtensions = function applyHttpExtensions(target, source) {
        for (var s in source) {
	...
	var action =  function($http,_emitter, _siteroot,_config){
