...
describe("Thenum", function(){
    ...
    before (function(done){
     ...
     describe ("can creates enumerations",function(){
       it("from arrays",function(){
           $([]).should.be.instanceOf($.Enumeration);
       });
        ...
        it("from numbers",function(){
            $(10).should.be.instanceOf($.Enumeration);
        });
        it("from nulls",function(){
            $(null).should.be.instanceOf($.Enumeration);
        });
        it("always new...",function(){
           var a  = [];
            var e = $(a);
            var e2 = $(a);
            e2.should.not.be.equal(e);
        });
        it("... except proxize enumerations",function(){
            var e = $([]);
            var e2 = $(e);
            e2.should.be.equal(e);
        });
    ...
    it("track state of next() and current value",function(){
        var i = $([1,3]);
        i.current.should.be.equal($.StartOfEnumeration);
        i.next().should.be.equal(true);
        i.current.should.be.equal(1);
        i.next().should.be.equal(true);
        i.current.should.be.equal(3);

        i.next().should.be.equal(false);
        i.current.should.be.equal($.EndOfEnumeration);
    ...
    try{
        if(navigator.userAgent.match(/firefox/i)){
            it("can be used with native iterator (only under Javascript 1.8)",function(){
                ...
                res.should.be.eql([ { key: 'x', value: 1 }, { key: 'y', value: 2 } ]);
            });
        }
    ...
    describe("can iterates over",function(){
        ...
        it("objects",function(){
            ...
            res.should.be.eql([{key:"a",value:1},{key:"b",value:2}]);
            res[0].should.be.instanceof($.KeyValuePair);
        });
        it("numbers",function(){
            var i = $(3);
            var res = [];
    ...
    it("has simple expression matcher",function(){
       var f1 = $.GetCondition(function(_){return _=="test"});
       var f2 = $.GetCondition("_=='test'");
       var f3 = $.GetCondition("=='test'");
       f1("test").should.be.equal(f2("test"));
       f1("test").should.be.equal(f3("test"));
        f1("notest").should.be.equal(f2("notest"));
        f1("notest").should.be.equal(f3("notest"));
    ...
    describe("is LINQ like",function(){
        it("supports any([condition])",function(){
            var i = $([1,2,3]);
            i.any().should.be.equal(true);
            i.any(">2").should.be.equal(true);
            i.any(">3").should.be.equal(false);
            i.any(function(_){return _==1}).should.be.equal(true);
        });
        it("supports toArray([condition])",function(){
            var i = $(4);
            i.toArray().should.be.eql([0,1,2,3]);
            i.toArray(">1").should.be.eql([2,3]);
        });
        it("supports count([condition])",function(){
           var i =  $(4);
            i.count().should.be.equal(4);
            i.count(">1").should.be.equal(2);
        });
        it("supports first([condition])",function(){
            var i =  $(4);
            i.first().should.be.equal(0);
            i.first(">1").should.be.equal(2);
            (function(){
                i.first(">5");
            }).should.throw();
        });
        ...
        it("supports each(func)",function(){
            ...
            res.should.be.eql([0,2]);
        });
        it("supports order([keyOrCompare])",function(){
            var i =  $([2,0,1,4,3]).order();
            i.toArray().should.be.eql([0,1,2,3,4]);
            i =  $([2,0,1,4,3]).order("-_");
            i.toArray().should.be.eql([4,3,2,1,0]);
            i =  $([2,0,1,4,3]).order(function(_){
                if(0==(_%2))return (_+1)*10;
                return _;
            });
            i.toArray().should.be.eql([1,3,0,2,4]);
            i =  $([2,0,1,4,3]).order(function(a,b){
                if(a==3 && b==4)return 1;
                if(a==4 && b==3)return -1;
                return a-b;
            });
            i.toArray().should.be.eql([0,1,2,4,3]);
        });
