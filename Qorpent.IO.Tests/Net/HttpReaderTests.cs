...
namespace Qorpent.IO.Tests.Net{
	...
	public class HttpReaderTests{
		...
		public void CanReadHeadersStack(){
			...
			Assert.AreEqual(
				"Version; State; StateName; HeaderName; HeaderValue; HeaderName; HeaderValue; HeaderName; HeaderValue; Finish; ",
				b.ToString());
		}
		...
		public void CanReadLiveSample(){
			...
			try{
				...
				using (var s = new NetworkStream(socket)){
					...
					ms.Position = 0;
					var content = Encoding.GetEncoding("utf-8").GetString(ms.GetBuffer(), 0, (int) ms.Length);
			...
			try{
				...
				using (var s = new NetworkStream(socket)){
					using (var ssl = new SslStream(s)){
						...
						ms.Position = 0;
						var content = Encoding.GetEncoding("Windows-1251").GetString(ms.GetBuffer(), 0, (int) ms.Length);
						//Console.WriteLine(content);
				...
				for (var i = 0; i < 20; i++){
					...
					var buff =
						Encoding.ASCII.GetBytes(
							"GET /Js/stub.en.js?v=82eb4b63730d HTTP/1.1\r\nHost: cdn.sstatic.net\r\nConnection: Keep-Alive\r\n\r\n");
					socket.Send(buff);
					var reciveCount = 0;
					var totalCount = 0;
