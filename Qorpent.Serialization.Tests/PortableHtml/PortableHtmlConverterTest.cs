...
<div>
  ...
  <p class=""c4"">Что же касается пугающих 50 рублей за доллар, то мы ожидаем определенной консолидации этих уровней: ухудшения позиций рубля ждать не стоит. Последнее связано с тем, что хоть курс рубля и зависит от цены на нефть практически напрямую, необходимо учитывать, что даже в худшие времена (цена нефти 40 долларов за баррель или даже исторические 12 долларов за баррель) курс рубля всегда находил опору.</p>
	    ...
	    [Test]
	    public void DigestGeneration() {
	        var phtml =
	            XElement.Parse(
	                @"<div><p>Екатеринбург, Сентябрь 08 (Новый День – Новый Регион, Валентина Ярославцева) – Сотрудница администрации Ивдельского городского округа лишилась квартиры, которую приобрела незаконным путем. Начальница отдела по жилищным вопросам воспользовалась своими служебными полномочиями и признала собственную семью малоимущей.</p><p>Как рассказали NDnews.ru в пресс-службе свердловского управления ФССП, в декабре 2013 года сотрудница администрации выделила себе трехкомнатную квартиру, основываясь на том, что жилье, в котором на тот момент официально проживали она и ее семья, является аварийным и не подлежит ремонту. Также он собрала документы, которые подтверждали якобы бедственное положение ее семьи, «забыв» при этом упомянуть о недавно приобретенном автомобиле и недвижимости. В итоге, 2014 год чиновница и ее семья встретили уже в новой квартире, а в марте того же года предприимчивая гражданка приватизировала жилье, переданное ей на условиях социального найма.</p><p>Однако, вскоре в администрации Ивдельского округа обнаружили подлог и начали судебное разбирательство. В суд был подан иск с требованием о выселении семьи чиновницы из квартиры и возврате жилья в муниципальный фонд. Суды первой и второй инстанции удовлетворили иск администрации. Уже после вынесенного судебного решения гражданка прописала в квартире своего несовершеннолетнего ребенка, надеясь, что это поможет ей сохранить крышу над головой. Несмотря на это, 26 августа судебные приставы-исполнители Ивдельского районного отдела выселили семью из спорной квартиры. При этой процедуре присутствовали сотрудники всех соответствующих служб: полиции, управляющей компании и городской администрации. Ключи от квартиры были переданы представителям муниципалитета на месте.</p></div>");
	        var digest = new PortableHtmlConverter().GetDigest(phtml, 400);
            Console.WriteLine(digest.Length);
            Assert.Greater(420,digest.Length);
	    }

	    [Test]
	    public void HighlightDigest() {
	        var phtml = XElement.Parse(@"
<div>
    <p>Это первая строка</p>
    <p>Всякий бутор</p>
    <p>Всякий бутор</p>
    <p>Всякий бутор</p>
    <p>Это <em>вторая</em> строка да вторая</p>
    <p>Это <em>вторая строка</em> строка</p>
    <p>Это первая строка вторая</p>
    <p>Это последняя строка</p>
</div>
");
	        var digest = new PortableHtmlConverter().GetDigest(phtml, 100, true);

             Assert.AreEqual(@"Это <em>вторая строка</em> да <em>вторая</em>",digest);
            digest = new PortableHtmlConverter().GetDigest(phtml, 400, true);
 Assert.AreEqual(@"Это первая строка... Это <em>вторая строка</em> да <em>вторая</em>... Это <em>вторая строка</em> строка", digest);
