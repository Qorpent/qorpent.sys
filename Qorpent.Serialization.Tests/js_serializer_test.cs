...
// 
// Solution: Qorpent
// Original file : js_serializer_test.cs
...
namespace Qorpent.Serialization.Tests {
	[TestFixture]
	public class js_serializer_test {
		private void test(object obj, string expected) {
			Assert.AreEqual(expected, new JsSerializer().DoSerialize(null, obj));
		}
		...
		[Test]
		public void anonymous_supported() {
			test(new {x = 1}, "{x: 1, }");
		}
		...
		[Test]
		public void bug_donot_escape_literals_with_dots_and_spaces() {
			test(new Dictionary<string, string> {{"x.x", "x"}}, "{'x.x': 'x', }");
			test(new Dictionary<string, string> {{"x x", "x"}}, "{'x x': 'x', }");
		}
		...
		public void complex_class_with_dictionary_and_array_serialized() {
			...
			test(obj, @"{name: 'x', dict: {x: 1, 'y\'x': true, z: 'test!', }, a: [1, true, 'test!', ], }");
		}
		...
		[Test]
		public void js_raw_supported() {
			test(new {x = (JsRaw) "function(){alert(1);}"}, "{x: function(){alert(1);}, }");
		}
