...
namespace Qorpent.Scaffolding.Tests.SqlAndOrm{
	...
	public class ExtendedScriptTest{
		...
		public void CanReadSimpleScript(){
			...
			var script = model.ExtendedScripts[0];
...
");
			var a = model.GetScripts(SqlDialect.PostGres, ScriptMode.Drop, ScriptPosition.Before).ToArray();
			Assert.AreEqual(1,a.Length);
			Assert.AreEqual("a",a[0].Name);
			var b = model.GetScripts(SqlDialect.SqlServer, ScriptMode.Create, ScriptPosition.After).ToArray();
